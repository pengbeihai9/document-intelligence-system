...
- revision_guidance用一段中文说明如何修改，必须具体。
...
def parse_top_venue_final_review(review_text: str) -> dict:
    ...
    return {
        ...
        "english_final": clean_summary_output(str(data.get("english_final", ""))),
    }
...
def apply_top_venue_final_review(api_key: str, api_base: str, models: list[str], source_text: str, summary: str, summary_mode: str, cn_target: int | None = None, en_target: int | None = None) -> tuple[str, str, str]:
    ...
    chinese_final = review.get("chinese_final", "")
    english_final = review.get("english_final", "")
    if not chinese_final:
        chinese_final = summary
    if summary_mode == "英文摘要":
        final_summary = english_final or summary
    ...
    return clean_summary_output(final_summary), used_model, format_top_venue_final_report(review)
...
def analyze_text(text: str, filename: str = "", summary_mode: str = "中文摘要", pdca_cycles: int = 3, pdca_pass_score: int = 88, cn_target: int | None = None, en_target: int | None = None) -> tuple[str, dict[str, int], str, str, str, pd.DataFrame, dict]:
    ...
    category = route["task_label"] if route.get("task_label") else category
