...
def init_storage() -> None:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        # 兼容已经部署过的旧数据库：新增字段用 ALTER TABLE 补齐，避免清库。
...
def local_english_fallback_summary(text: str, key_sentences: list[str], keywords: str, academic: bool, material_style: str) -> str:
    """在离线兜底场景下生成保守英文摘要，确保页面始终同时展示中英文结果。"""
    source = normalize_summary_input(text)
    # 优先复用原文里的英文句子；没有英文信息时只给保守概述，避免本地规则编造细节。
    english_sentences = [
        sentence for sentence in split_sentences(source)
        if len(re.findall(r"[A-Za-z]{3,}", sentence)) >= 6
    ]
    if english_sentences:
        return clean_english_summary_output(compact_sentences(english_sentences[:5], limit=900))

    material_name = "presentation or visual material" if material_style in {"presentation", "visual"} else "document"
    keyword_text = keywords.replace("、", ", ")
    if academic:
        return (
            f"This {material_name} focuses on {keyword_text}. The available text describes the main research topic, "
            "related methods or analytical context, and the conclusions that can be supported by the extracted content. "
            "Because this summary is generated by the local fallback workflow, it avoids adding sample sizes, metrics, "
            "experimental results, or policy implications that are not explicitly present in the source."
        )
    return (
        f"This {material_name} focuses on {keyword_text}. The extracted content is organized around the main topic, "
        "key information, possible caveats, and actionable points that are visible in the source. The local fallback "
        "summary only uses identifiable text and does not infer unavailable details."
    )


def local_bilingual_summary(chinese_summary: str, text: str, key_sentences: list[str], keywords: str, academic: bool, material_style: str) -> str:
    """组合本地中文摘要和英文兜底摘要，格式与在线双语摘要保持一致。"""
    english_summary = local_english_fallback_summary(text, key_sentences, keywords, academic, material_style)
    return f"{clean_summary_output(chinese_summary)}\n\nEnglish Summary\n{english_summary}"
...
def summarize_text(text: str, max_sentences: int = 5, summary_mode: str = "中文摘要", material_style: str = "document") -> str:
    ...
    # PPT 和图片 OCR 往往是碎片短句，单独走更保守的材料说明模板。
    if material_style in {"presentation", "visual"}:
        ...
        if summary_mode == "双语摘要":
            return local_bilingual_summary(summary, text, key_sentences, keywords, False, material_style)
        if summary_mode == "英文摘要":
            return local_english_fallback_summary(text, key_sentences, keywords, False, material_style)
        return summary
    ...
    if is_academic_document(text):
        ...
        if summary_mode == "双语摘要":
            return local_bilingual_summary(summary, text, key_sentences, keywords, True, material_style)
        if summary_mode == "英文摘要":
            return local_english_fallback_summary(text, key_sentences, keywords, True, material_style)
        return summary
...
def classify_document(text: str) -> tuple[str, dict[str, int]]:
    ...
    # 分类逻辑保持可解释：每个标准类别只按关键词累计打分，页面会展示命中依据。
    scores = {
...
def build_wordcloud(freq_df: pd.DataFrame, top_n: int = 40):
    ...
    # 词云对空值、停用词和低频词比较敏感，先清洗再绘图，失败时返回占位图。
...
def parse_uploaded_file(uploaded_file, ocr_mode: str = "standard", pdf_extract_mode: str = "auto") -> tuple[str, bool]:
    ...
    # 同一入口统一分发到不同解析器；解析后再做页眉页脚、断行和 OCR 噪声清洗。
    if suffix == "pdf":
...
def analyze_text(text: str, filename: str = "", summary_mode: str = "双语摘要", pdca_cycles: int = 3, pdca_pass_score: int = 88, cn_target: int | None = None, en_target: int | None = None, quality_mode: str = "快速") -> tuple[str, dict[str, int], str, str, str, pd.DataFrame, dict]:
    """对提取文本执行分类、摘要和词频统计。"""
    route = route_file(filename, text).to_dict()
    material_style = infer_material_style(filename, route.get("file_type", ""))
    # 分析层只负责编排：分类、摘要、双语拆分和词频统计分别由独立函数完成。
...
with st.sidebar:
    ...
    if not documents:
        ...
        saved_categories = sorted({doc["category"] for doc in documents if doc["category"]})
        categories = ["全部文档", *saved_categories]
        selected_category = st.radio("按分类筛选", categories, horizontal=False, index=0)
        filtered_documents = [doc for doc in documents if selected_category == "全部文档" or doc["category"] == selected_category]
...
if uploaded_file is not None:
    ...
    upload_signature = (
        ...
        f"{st.session_state.get('summary_mode', '双语摘要')}:"
...
if uploaded_file is not None and st.session_state.processed_upload != upload_signature:
    ...
    eta_hint = estimate_processing_time(
        ...
        st.session_state.get("summary_mode", "双语摘要"),
    )
...
st.success(f"系统分类结果：{result['category']}")
